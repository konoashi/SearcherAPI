...
import static fr.konoashi.searcher.App.datacheck;
...
public class Searcher {
    private void CheckStuff(JsonElement data, String uuid) {
        for (int i = 0; i < data.getAsJsonObject().getAsJsonArray("profiles").size(); i++) {
            ...
            if(member.getAsJsonObject().getAsJsonArray("pets") != null) {
                PET(member.getAsJsonObject().getAsJsonArray("pets"), profile.getAsJsonObject().get("cute_name").getAsString(), UUID.fromString(uuid));
            }
    ...
    private void PET(JsonElement data, String profile, UUID uuid) {

        ArrayList<String> type = new ArrayList<>();
        for (int i = 0; i < data.getAsJsonArray().size(); i++) {
            type.add(data.getAsJsonArray().get(i).getAsJsonObject().get("type").getAsString());
        }
        ArrayList<String> tier = new ArrayList<>();
        for (int i = 0; i < data.getAsJsonArray().size(); i++) {
            tier.add(data.getAsJsonArray().get(i).getAsJsonObject().get("tier").getAsString());
        }
        for (int i = 0; i < tier.size(); i++) {
            if (Objects.equals(tier.get(i), "RARE")) {
                if (Objects.equals(type.get(i), "PIGMAN")) {
                    JsonObject prop = new JsonObject();
                    prop.addProperty("Found", "Rare Pigman PET");
                    prop.addProperty("UUID", String.valueOf(uuid));
                    prop.addProperty("Profile", profile);
                    prop.addProperty("Location", "Pet Menu");
                    datacheck.add(prop);
                }
                if (Objects.equals(type.get(i), "BLAZE")) {
                    JsonObject prop = new JsonObject();
                ...
                if (Objects.equals(type.get(i), "WITHER_SKELETON")) {
                    JsonObject prop = new JsonObject();
            ...
            if (Objects.equals(tier.get(i), "COMMON")) {
                if (Objects.equals(type.get(i), "SNOWMAN")) {
                    JsonObject prop = new JsonObject();
